...
int screenshot_save(char *out, int out_size)
{
    ...
    put32(head + 38, 2835);
    put32(head + 42, 2835);
    if (sys_write(h, head, 54) != 54) { free(row); sys_close(h); return -1; }
    ...
    for (y = scr_h - 1; y >= 0; y--) {
        ...
        if (sys_write(h, row, stride) != stride) { free(row); sys_close(h); return -1; }
    }
    free(row);
    sys_close(h);
    return 0;
